...
export default function BenchmarkHeader({ task, author }: BenchmarkHeaderProps) {
  ...
  return (
    <div>
      {/* Title */}
      <h1 className="text-3xl font-medium text-[#e7e9ea] tracking-tight leading-tight">
        {task.title}
      </h1>
      ...
      <div className="mt-4 flex items-center gap-3">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          {task.difficulty && diffStyle && (
            <span
              className={`inline-flex items-center gap-1.5 rounded-full border px-2.5 py-0.5 text-xs font-medium capitalize ${diffStyle.text} ${diffStyle.bg}`}
            >
          ...
          {task.difficulty && !diffStyle && (
            <span className="rounded-full px-2.5 py-0.5 text-xs font-medium capitalize text-[#8b8d93] bg-white/[0.04] border border-white/[0.06]">
              {task.difficulty}
            </span>
          )}
        </div>
      ...
      {/* Description */}
      <p className="mt-4 text-sm text-[#8b8d93] leading-relaxed max-w-3xl">
        {task.description}
      </p>
      ...
      {task.tags && task.tags.length > 0 && (
        <div className="mt-4 flex flex-wrap items-center gap-2">
          {task.tags.map((tag) => (
            <span
              key={tag}
              className="rounded-full px-3 py-1 text-xs font-medium text-[#8b8d93] bg-white/[0.04] border border-white/[0.04] hover:bg-white/[0.06] transition-colors duration-150"
            >
              {tag}
            </span>
          ))}
